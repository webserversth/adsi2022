...
class Paginas extends Controllers{
	...
	public function editar($idpost){
		if(empty($_SESSION['permisosMod']['Perm_update'])){
			header("Location:".base_url().'/dashboard');
		}
	...
	public function getPaginas(){
		if($_SESSION['permisosMod']['Perm_read']){
			...
			for ($i=0; $i < count($arrData); $i++) {
				...
				$btnEdit = '';
				$btnDelete = '';
				$urlPage = base_url()."/".$arrData[$i]['Pag_Ruta'];
				...
				if($_SESSION['permisosMod']['Perm_update']){
					$btnEdit = '<a title="Editar página" href="'.base_url().'/paginas/editar/'.$arrData[$i]['IdPagina'].'" class="btn btn-primary btn-sm"> <i class="fas fa-pencil-alt"></i></a>';
				}
				if($_SESSION['permisosMod']['Perm_delete']){	
					$btnDelete = '<button class="btn btn-danger btn-sm" onClick="fntDelInfo('.$arrData[$i]['IdPagina'].')" title="Eliminar página"><i class="far fa-trash-alt"></i></button>';
				}
