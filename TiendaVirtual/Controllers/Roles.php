	...
	class Roles extends Controllers{
		...
		public function getRoles()
		{
			if($_SESSION['permisosMod']['Perm_read']){
				...
				//dep($arrData);exit;  // // ayuda Muestra la informacion del Json ingresando al a la url http://localhost/adsi/DesarrolloTiendaVirtual_V2/parametros/getParametros
		...
		public function getSelectRoles()
		{
			...
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['Est_IdEstado'] == 1 ){
					$htmlOptions .= '<option value="'.$arrData[$i]['idrol'].'">'.$arrData[$i]['Rol_Nombre'].'</option>';
					}
				}
			}
			echo $htmlOptions;
