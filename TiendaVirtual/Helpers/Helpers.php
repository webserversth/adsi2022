<?php 
    ...
    //Formato para valores monetarios   colocar  para  despues de $cantidad para mostrar  cifras decimales
    ...
    function getInfoPage(int $idpagina){
        ...
        $con = new Mysql();
        $sql = "SELECT * FROM TblPagina WHERE IdPagina = $idpagina";
    ...
    function getPageRout(string $ruta){
        require_once("Libraries/Core/Mysql.php");
        $con = new Mysql();
        $sql = "SELECT * FROM TblPagina WHERE Pag_Ruta = '$ruta' AND Est_IdEstado != 0 ";
        $request = $con->select($sql);
        if(!empty($request)){
            $request['Pag_Portada'] = $request['Pag_Portada'] != "" ? media()."/images/uploads/".$request['Pag_Portada'] : "";
        }
        return $request;
    }
    ...
    function viewPage(int $idpagina){
        ...
        $sql = "SELECT * FROM TblPagina WHERE IdPagina = $idpagina ";
        $request = $con->select($sql);
        if( ($request['Est_IdEstado'] == 2 AND isset($_SESSION['permisosMod']) AND $_SESSION['permisosMod']['Perm_update'] == true) OR $request['Est_IdEstado'] == 1){
            return true;        
        }else{
            return false;
        }
