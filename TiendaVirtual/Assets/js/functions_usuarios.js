...
function fntViewUsuario(idpersona){
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
               let estadoUsuario = objData.data.Est_IdEstado == 1 ? 
                '<span class="badge badge-success">Activo</span>' : 
                '<span class="badge badge-danger">Inactivo</span>';

                document.querySelector("#celIdentificacion").innerHTML = objData.data.Per_Identificacion;
                document.querySelector("#celNombre").innerHTML = objData.data.Per_Nombres;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector("#txtNombre").value = objData.data.Per_Nombres;
